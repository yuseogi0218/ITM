...
public class AddressTranslator {
	...
	public static void main(String[] args) {
		...
		try {
				...
				if (f_num == -1) {
					...
					if (f_num == -1) {
						...
						if (pm.currentFreeFrame == 128) {
							page_replacement++;
							pm.FIFO(pt.pagelist.get(0)); // delete first input frame - Victim in PageTable.
							pt.FIFO(pt.pagelist.get(0)); // change (FrameNumber:-1, Validation:false) of Victim in
															// PageTable.
						}
			...
			System.out.println(String.format("TLB miss: %s, Page Fault: %s, Page Replacement: %s", tlb_miss, page_fault,
					page_replacement));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
